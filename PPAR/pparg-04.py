...
def assignFingerprint( moles, nBits=2048 ) :
        df = pd.DataFrame( columns=[ 'name', 'pChEMBL_Value', 'fingerprint' ] )
        for m in moles:
                df.append( { 'name' : m.GetProp( 'Molecule ChEMBL ID' ),
                        'pChEMBL_Value' : m.GetProp( 'pChEMBL_Value' ),
                        'fingerprint' : Chem.rdMolDescriptors.GetMorganFingerprintAsBitVect( m, radius=2, nBits=nBits ) },
            ignore_index=True )
        return df
...
def main() :
        ...
        if False :              # classification
                ...
                sdfname = "./2001609-ChEMBL-2672.sdf"
                moles = readMolecules( sdfname )
                assignFingerprint( moles, nBits )
